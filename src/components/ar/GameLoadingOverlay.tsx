...
import { Animated, View, Text, TouchableOpacity, ScrollView, SafeAreaView } from 'react-native';
...
const STEPS = [
  { icon: 'phone-portrait-outline', title: 'Look Around',  desc: 'Move your phone to explore your space' },
  { icon: 'search-outline',         title: 'Find It',      desc: 'Find the 3D object matching the word' },
  { icon: 'hand-left-outline',      title: 'Tap to Get',   desc: 'Tap the correct object for +10 pts' },
  { icon: 'flash-outline',          title: 'Be Quick!',    desc: 'Wrong taps cost 5 pts. 60 seconds!' },
];

export const GameLoadingOverlay = ({
  loadFadeAnim, loadedWords, allLoaded, totalModels, allWords, onStartPlay,
}: Props) => (
  <Animated.View style={[s.overlay, { opacity: loadFadeAnim }]} pointerEvents="auto">
    ...
    <ScrollView style={s.scroll} contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={s.emojiGrid}>
        {allWords.map(word => {
          ...
          return (
            <View key={word} style={[s.emojiCell, loaded && s.emojiCellLoaded]}>
      ...
      {!allLoaded ? (
        <View style={s.progressContainer}>
          <Text style={s.progressSubtitle}>Placing objects in your room...</Text>
          <View style={s.progressBarTrack}>
            <View style={[s.progressBarFill, { width: `${(loadedWords.length / totalModels) * 100}%` }]} />
          </View>
          <Text style={s.progressCount}>{loadedWords.length}/{totalModels} ready</Text>
        </View>
      ) : (
        <TouchableOpacity
          style={s.startBtn}
      ...
      <View style={s.tipRow}>
        <Ionicons name="bulb-outline" size={14} color="rgba(255,255,255,0.45)" />
        <Text style={s.tipText}>Good lighting helps you find objects faster!</Text>
      </View>
