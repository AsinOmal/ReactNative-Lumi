...
export const ScanAndCountScene = (props: any) => {
  const {
    targetWord, targetCount, distractors, foundIndices,
    positions, onTargetTap, onDistractorTap, onModelLoaded,
  ...
  return (
    <ViroARScene>
      ...
      {/* Target instances */}
      {model && Array.from({ length: targetCount }, (_, i) => {
        ...
        return (
          <ViroNode
            ...
            opacity={isFound ? 0 : 1}
            onClickState={(s: number) => s === 1 && withDebounce(() => onTargetTap(i))}
            animation={{ name: 'rotate', run: !isFound, loop: true }}
          >
            <Viro3DObject source={model.source} type="GLB" scale={model.scale} onLoadEnd={() => onModelLoaded(true)} />
            <ViroText
              text={isFound ? '✓' : targetWord.charAt(0).toUpperCase() + targetWord.slice(1)}
              position={[0, 0.15, 0]}
              style={{ fontStyle: 'Normal', fontSize: 12, color: isFound ? '#22C55E' : '#FBBF24' }}
      ...
      {(distractors as string[]).map((word: string, i: number) => {
        ...
        return (
          <ViroNode
            ...
            opacity={0.6}
            onClickState={(s: number) => s === 1 && withDebounce(() => onDistractorTap(word))}
            animation={{ name: 'rotate', run: true, loop: true }}
          >
            <Viro3DObject source={dm.source} type="GLB" scale={dm.scale} onLoadEnd={() => onModelLoaded(false)} />
            <ViroText
