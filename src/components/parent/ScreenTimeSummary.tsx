...
export const ScreenTimeSummary: React.FC = () => {
  ...
  const handleSetLimit = () => {
    if (user) {
      updateSettings(user.uid, { dailyLimitMinutes: localLimit });
    }
  };

  const handleToggleTimedMode = (val: boolean) => {
    if (user) {
      updateSettings(user.uid, { timedModeEnabled: val });
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.card, styles.limitRow]}>
        ...
        <Text style={styles.limitValue}>
          {localLimit === 0
            ? strings.screenTimeNoLimit
            : strings.screenTimeLimitFmt(localLimit)}
        </Text>
        <TouchableOpacity
          style={[
            styles.stepBtn,
            localIdx === LIMIT_STEPS.length - 1 && styles.stepBtnDisabled,
          ]}
          onPress={() =>
            setLocalIdx((i) => Math.min(LIMIT_STEPS.length - 1, i + 1))
          }
      ...
      >
        <Ionicons
          ...
          size={16}
          color={colors.white}
          style={styles.setBtnIcon}
        />
        <Text style={styles.setBtnText}>
          {hasUnsavedChanges ? 'Set Limit' : 'Limit Saved'}
        </Text>
      </TouchableOpacity>
      ...
      <View style={[styles.card, styles.timedRow]}>
        ...
        <View style={styles.timedText}>
          <Text style={styles.timedLabel}>
            {strings.screenTimeTimedModeLabel}
          </Text>
