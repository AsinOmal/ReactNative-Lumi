...
export const useMakeAMeal = ({ recipe, onComplete }: UseMakeAMealProps) => {
  ...
  useEffect(() => {
    if (!recipe) { setSpawnWords([]); return; }
  ...
  const handleIngredientTap = useCallback((word: string) => {
    if (isTapping.current || collectedIngredients.includes(word) || !recipe) return;
  ...
  const handleDistractorTap = useCallback((_word: string) => {
  ...
  return { spawnWords, feedback, feedbackAnim, handleIngredientTap, handleDistractorTap };
};
